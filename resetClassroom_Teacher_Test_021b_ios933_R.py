...
class TestTeacher(unittest.TestCase):
    ...
    def resetClassroom(self):
        ...
        if len(o)!=0:
            ...
            sleep(3)
        now=time.strftime('%Y-%m-%d %H_%M_%S')
        sf0='./'+now+'_021b_beforeReset_R.png'
        driver.save_screenshot(sf0)
        sleep(2)
        driver.find_element_by_accessibility_id('更多').click()
        sleep(3)
        driver.find_element_by_accessibility_id('重置教室').click()
        sleep(4)
        now=time.strftime('%Y-%m-%d %H_%M_%S')
        sf1='./'+now+'_021b_afterReset_R.png'
...
if __name__ == '__main__':
    ...
    testunit.addTest(TestTeacher('resetClassroom'))
    now=time.strftime('%Y-%m-%d %H_%M_%S')
    filename='./'+now+'_021b_R.html'
    fp=open(filename,'wb')
    runner=HTMLTestRunner(stream=fp,title='测试老师版iOSiOS9.3.3/iPhone5s真机(重置教室)测试报告by Appium',
