...
class TestTeacher(unittest.TestCase):
    ...
    def waitStuCheckMusic(self):
        ...
        if len(lis1)!=0:
            ...
            #历史课单 top
            driver.swipe(500,400,0,-184,1000)
            sleep(2)
        ...
        if i==0:
            ...
            if len(self)!=0:
                ...
                plist=driver.find_elements_by_accessibility_id('ic next1')
                if len(plist)!=0:
                ...
                if len(flag)==0:
                    ...
                    now=time.strftime('%Y-%m-%d %H_%M_%S')
                    sf2='./'+now+'_031b_editNotAllowed_R.png'
                    driver.save_screenshot(sf2)
                    sleep(2)
                    print('课程结束后不允许编辑乐谱')
                    sleep(1)
...
if __name__ == '__main__':
    ...
    fp=open(filename,'wb')
    runner=HTMLTestRunner(stream=fp,title='测试老师版iOS9.3.3/iPhone5s真机(等待学生查看上课乐谱)测试报告by Appium',
