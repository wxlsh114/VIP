...
class TestTeacher(unittest.TestCase):
    ...
    def checkHistoryClass(self):
        ...
        if len(lis1)!=0:
            for i in range(6):
                ...
                if bu.text!=0:
                    ...
                    sleep(2)
                    #历史课单 middle
                    driver.find_element_by_android_uiautomator('new UiSelector().text("历史课单")').click()
                    sleep(2)
                    break
        else:
            driver.find_element_by_android_uiautomator('new UiSelector().text("历史课单")').click()
        ...
        driver.find_elements_by_android_uiautomator('new UiSelector().text("查看课单")')[0].click()
        sleep(10)
        now=time.strftime('%Y-%m-%d %H_%M_%S')
...
if __name__ == '__main__':
    ...
    fp=open(filename,'wb')
    runner=HTMLTestRunner(stream=fp,title='测试老师版android7.0真机(Honor8Lite)[查看历史课单]测试报告by Appium',
