...
class TestStudent(unittest.TestCase):
    ...
    def deleteEditMusic(self):
        ...
        c.screenshot(sf)
        sleep(2)
        if ((s(id='查看乐谱').exists) and (not ('已开始' in flag) and not ('已结束' in flag))):
            s(id='查看乐谱').tap()
            sleep(2)
            if s.alert.wait(3):
                s.alert.accept()
...
if __name__ == '__main__':
    ...
    fp=open(filename,'wb')
    runner=HTMLTestRunner(stream=fp,title='测试学生版iOS11.0.3/iPhone6真机(非课程开始和结束后时间段删除/编辑曲谱)测试报告by WDA(Facebook)',
