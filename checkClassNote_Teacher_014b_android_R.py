        ...
        if len(lis1)!=0:
            for i in range(5):
                ...
                if bu.text!='0':
                    ...
                    sleep(2)
                    #课程要求 middle
                    driver.find_element_by_id('com.pnlyy.pnlclass_teacher.test:id/lookDesTv').click()
                    sleep(2)
                    break
        else:
            driver.find_element_by_id('com.pnlyy.pnlclass_teacher.test:id/lookDesTv').click()
...
if __name__ == '__main__':
    ...
    testunit.addTest(TestTeacher('checkClassNote'))
    now=time.strftime('%Y-%m-%d %H_%M_%S')
    filename='./'+now+'_014b_result_R.html'
    fp=open(filename,'wb')
    runner=HTMLTestRunner(stream=fp,title='测试老师版android7.0真机(Honor8Lite)[查看课程要求]测试报告by Appium',
