...
class TestTeacher(unittest.TestCase):
    ...
    def resetClassroom(self):
        ...
        sleep(3)
        driver.find_element_by_android_uiautomator('new UiSelector().text("重置教室")').click()
        sleep(3)
        driver.find_element_by_android_uiautomator('new UiSelector().text("确定")').click()
        sleep(4)
        now=time.strftime('%Y-%m-%d %H_%M_%S')
        sf0='./'+now+'_021b_afterReset_R.png'
...
if __name__ == '__main__':
    ...
    fp=open(filename,'wb')
    runner=HTMLTestRunner(stream=fp,title='测试老师版android7.0真机(Honor8Lite)[重置教室]测试报告by Appium',
