...
class TestStudent(unittest.TestCase):
    ...
    def deleteEditMusic(self):
        ...
        if len(lis1)==0:
            ...
            sleep(3)
            now=time.strftime('%Y-%m-%d %H_%M_%S')
            sf1='./'+now+'_025b_classMuisc_R.png'
            ...
            if (i!=0 and (not ('已开始' in flag) and not ('已结束' in flag))):
                ...
                if len(self)!=0:
                    ...
                    sleep(5)
                    now=time.strftime('%Y-%m-%d %H_%M_%S')
                    sf1='./'+now+'_025b_musicBySelfDetail_R.png'
                    driver.save_screenshot(sf1)
                    sleep(2)
...
if __name__ == '__main__':
    ...
    now=time.strftime('%Y-%m-%d %H_%M_%S')
    filename='./'+now+'_025b_result_R.html'
    fp=open(filename,'wb')
    runner=HTMLTestRunner(stream=fp,title='测试学生版android7.0真机(Honor8Lite)[非课程开始和结束后时间段删除/编辑曲谱]测试报告by Appium',
